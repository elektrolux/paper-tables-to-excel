...
- 保留源表中对应的 p 值数据列，列内只保留原文报告的数值或空白；删除显著性星号及附着于 p 值的其他非数值符号，不为显著性符号另建列，也不推算或重算 p 值。若原文只有显著性符号而没有数值型 p 值列，则删除符号且不生成 p 值。
- 置信区间拆为独立的下限列和上限列。
...
4. 源表中的 p 值数据列均已保留为数值列，单元格只含数值或空白，且不含显著性星号或其他非数值符号；表头不含来源字段。
